...
def get_alerts():
    ...
    for alert in alerts:
        ...
        rew_items = ' - ' + L[alert['MissionInfo']['missionReward']['items'][0].lower(
...
def get_invasions():
    ...
    for invasion in invasions:
        ...
        atk_reward = L[invasion['AttackerReward']['countedItems'][0]['ItemType'].lower(
        ...
        def_reward = L[invasion['DefenderReward']['countedItems'][0]['ItemType'].lower(
...
def get_riven_info(weapon, simulate=0):
    ...
    if weapon in melee_dispo or weapon in rifle_dispo or weapon in shotgun_dispo or weapon in pistol_dispo:
        ...
        riven_info = riven_details(random.sample(list(melee_dispo), 1)[
                                   0], random.randint(2, 3), random.randint(0, 1))
    elif roll >= 0.3915 and roll < 0.6853:
        prefix = '你从虚空中获得了一张手枪裂罅Mod并开出了：\n'
        riven_info = riven_details(random.sample(list(pistol_dispo), 1)[
                                   0], random.randint(2, 3), random.randint(0, 1))
    elif roll >= 0.6853 and roll < 0.9475:
        prefix = '你从虚空中获得了一张步枪裂罅Mod并开出了：\n'
        riven_info = riven_details(random.sample(list(rifle_dispo), 1)[
                                   0], random.randint(2, 3), random.randint(0, 1))
    elif roll >= 0.9475:
        prefix = '你从虚空中获得了一张霰弹枪裂罅Mod并开出了：\n'
        riven_info = riven_details(random.sample(list(shotgun_dispo), 1)[
                                   0], random.randint(2, 3), random.randint(0, 1))
    else:  # how???
        pass
    if weapon == '卡':
